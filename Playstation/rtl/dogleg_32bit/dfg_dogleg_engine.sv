// =============================================================================
// File Name   : dfg_dogleg_engine.sv
// Module Name : dfg_dogleg_engine
// Project     : Trust-Region Dogleg Non-Linear Optimizer Accelerator (Solver #13)
// -----------------------------------------------------------------------------
// Description:
//   Programmable DFG Model Evaluator for Trust-Region Dogleg optimization.
//   Evaluates f(t_m, x) given observation t_m (loaded into r0) and parameter
//   vector x (loaded into r1..r4). Output prediction is returned in r15.
// =============================================================================
            ...
            case (state)
                ENG_IDLE: begin
                    ...
                    if (start_eval) begin
                        busy <= 1'b1;
                        pc   <= '0;
                        reg_file[REG_T_VAL] <= t_in;
                        for (int i = 0; i < MAX_PARAMS; i++) begin
                            if (i < num_params) begin
                                reg_file[i + 1] <= get_vec(x_vec, 2'(i));
                            end
                        end
