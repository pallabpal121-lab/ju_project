// =============================================================================
// File Name   : dogleg_jacobian_engine.sv
// Module Name : dogleg_jacobian_engine
// Project     : Trust-Region Dogleg Non-Linear Optimizer Accelerator (Solver #13)
// -----------------------------------------------------------------------------
// Description:
//   Evaluates residual vector r (M x 1), central finite-difference Jacobian J (M x N),
//   normal Hessian Gram matrix B = JᵀJ + λ_eps·I (N x N), gradient vector g = Jᵀr (N x 1),
//   and total least-squares objective cost S(x) = 0.5 * sum(r_m^2).
// =============================================================================
...
module dogleg_jacobian_engine (
    ...
    output mat_t               mat_a,       // Hessian approximation B = JᵀJ + λ_eps*I (N x N)
    output logic               done,
    output logic               busy
);
        ...
        if (!rst_n) begin
            ...
            case (state)
                ...
                J_ACCUM: begin
                    ...
                    // 3. Normal Hessian matrix B_jk = sum_m (J_mj * J_mk) + (j==k ? λ_eps : 0)
                    out_a = '0;
                    for (int j = 0; j < MAX_PARAMS; j++) begin
                        if (j < num_params[1:0]) begin
