// =============================================================================
// File Name   : lm_types_pkg.sv
// Module Name : lm_types_pkg (SystemVerilog Package)
// Project     : Levenberg-Marquardt (LM) Non-Linear Least Squares Accelerator (Solver #2)
// -----------------------------------------------------------------------------
// Description for Freshers & Beginners:
//   This package defines all vector, matrix, and observation dataset data types
//   used by the Levenberg-Marquardt Non-Linear Least Squares Hardware Accelerator.
//
// Key Concepts:
//   - Parameter Vector x : [x0, x1, x2, x3] (up to MAX_PARAMS = 4)
//   - Observations (t_m, y_m) : Measurement data points (up to MAX_OBS = 8)
//   - Residual Vector r : r_m = f(t_m, x) - y_m
//   - Jacobian Matrix J : J_mn = ∂r_m / ∂x_n
//   - Normal Equations  : (Jᵀ · J + λ · I) · p = -Jᵀ · r
// =============================================================================

`timescale 1ns / 1ps

package lm_types_pkg;
    ...
    localparam q16_t Q16_LAMBDA_DEF  = 32'h0000_0800; // Default initial damping λ ≈ 0.03125
    localparam q16_t Q16_LAMBDA_MIN  = 32'h0000_0020; // Minimum damping λ_min ≈ 0.0005
    localparam q16_t Q16_LAMBDA_MAX  = 32'h0040_0000; // Maximum damping λ_max = 64.0
    ...
    //   - r0..r(N-1) : Parameter vector x[0..N-1]
    //   - r14        : Observation input t_m
    //   - r15        : Evaluated model result f(t_m, x)
    localparam int REG_T_IN   = 14;
    ...
    typedef enum logic [2:0] {
        STATUS_IDLE        = 3'd0,
        STATUS_RUNNING     = 3'd1,
        STATUS_CONVERGED   = 3'd2, // ||g|| <= tolerance or cost change <= tol
