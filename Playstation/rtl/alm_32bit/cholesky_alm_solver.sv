// =============================================================================
// File Name   : cholesky_alm_solver.sv
// Module Name : cholesky_alm_solver
// Project     : Augmented Lagrangian Method (ALM) Accelerator (Solver #21)
// -----------------------------------------------------------------------------
// Description:
//   Solves the symmetric positive-definite augmented system:
//   H_aug * Δx = -g_aug
        ...
        if (!rst_n) begin
            ...
            case (state)
                ...
                CH_INIT: begin
                    ...
                    // Add small regularizing epsilon if needed
                    sqrt_rad   <= (h_elem - acc_sum > Q16_DAMP_DEF) ? (h_elem - acc_sum) : Q16_DAMP_DEF;
                    sqrt_start <= 1'b1;
                    state      <= CH_DIAG_WAIT;
                end
                ...
                CH_FWD_START: begin
                    if (i_idx < num_dims) begin
                        ...
                        for (int k = 0; k < MAX_DIM; k++) begin
                            if (k < i_idx) begin
                                l_ik    = get_mat(l_mat, 2'(i_idx), 2'(k));
                                acc_sum = acc_sum + q16_mul(l_ik, y_vec[k]);
                            end
                        end
                        div_dividend <= -g_vec[i_idx] - acc_sum;
                ...
                CH_BWD_START: begin
                    ...
                    for (int k = 0; k < MAX_DIM; k++) begin
                        if (k > i_idx && k < num_dims) begin
                            ...
                            acc_sum = acc_sum + q16_mul(l_ik, dx_reg[k]);
                        end
                    end
                    div_dividend <= y_vec[i_idx] - acc_sum;
                    div_divisor  <= get_mat(l_mat, 2'(i_idx), 2'(i_idx));
                    div_start    <= 1'b1;
                    state        <= CH_BWD_WAIT;
                end
