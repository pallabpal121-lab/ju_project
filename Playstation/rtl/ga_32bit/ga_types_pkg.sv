// =============================================================================
// File Name   : ga_types_pkg.sv
// Module Name : ga_types_pkg (SystemVerilog Package)
...
//   gene vectors, fitness arrays, objective function selectors, and status codes.
// =============================================================================

`timescale 1ns / 1ps

package ga_types_pkg;
    ...
    // -------------------------------------------------------------------------
    // SECTION 2: POPULATION & GENE DIMENSIONS (P <= 8, D <= 4)
    // -------------------------------------------------------------------------
    localparam int MAX_POP           = 8; // Up to 8 individuals
    localparam int MAX_DIM           = 4; // Up to 4 continuous parameters

    // Packed 4-element single individual gene/chromosome vector x
    ...
    typedef enum logic [1:0] {
        ...
        FN_SPHERE      = 2'd2, // f(x) = ∑ (x_d - d)^2
        FN_RASTRIGIN   = 2'd3  // f(x) = ∑ (x_d^2 - 0.5 cos(2π x_d))
    } fitness_fn_t;
    ...
    typedef enum logic [2:0] {
        STATUS_IDLE         = 3'd0,
        STATUS_EVALUATING   = 3'd1, // Evaluating population fitness
        STATUS_REPRODUCING  = 3'd2, // Selection, Crossover, and Mutation active
        STATUS_CONVERGED    = 3'd3, // Fitness below target epsilon
        STATUS_MAX_GENS     = 3'd4, // Maximum generations reached
        STATUS_ERROR        = 3'd5
    } status_t;

endpackage : ga_types_pkg
