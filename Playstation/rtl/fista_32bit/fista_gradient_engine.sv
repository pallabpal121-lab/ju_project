// =============================================================================
// File Name   : fista_gradient_engine.sv
// Module Name : fista_gradient_engine
// Project     : Fast Iterative Shrinkage-Thresholding Algorithm (FISTA) Accelerator (Solver #15)
// -----------------------------------------------------------------------------
// Description:
//   Evaluates the numerical gradient g = ∇f(y) at the extrapolated momentum
//   point y using central finite differences:
//   g_i = (f(y + h*e_i) - f(y - h*e_i)) / (2*h)
//   With h = 2^-4 = 0.0625, 1/(2h) = 2^3 = 8 (single-cycle arithmetic left shift <<< 3).
//   Also returns base cost f(y).
// =============================================================================
...
module fista_gradient_engine (
    ...
    input  vec_t               y_in,          // Extrapolated momentum vector y_k

    // Outputs
    output vec_t               vec_g_out,     // Gradient vector g = ∇f(y)
    output q16_t               f_base_out,    // Cost f(y)
    output logic               done,
    output logic               busy
);
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            case (state)
                ...
                // 2. Evaluate f(y + h*e_i)
                G_START_POS: begin
                    orig_val = get_vec(y_in, 2'(curr_dim));
                    dfg_x_in <= set_vec(y_in, 2'(curr_dim), orig_val + Q16_H_STEP);
                    start_dfg <= 1'b1;
                    state     <= G_WAIT_POS;
                end
