...
//   Solves the N-dimensional linear system (H + λI) · p = -g using hardware
//   Cholesky Decomposition (A = L · Lᵀ) followed by forward and backward substitution.
// =============================================================================
...
module cholesky_solver_engine (
    ...
    input  mat_t               mat_a,       // Input matrix A = H + lambda*I (N x N)
    input  vec_t               vec_g,       // Gradient vector g (N x 1)

    // Outputs
    output vec_t               vec_p,       // Computed Newton search direction p (N x 1)
    output logic               done,        // 1-cycle completion strobe
    output logic               singular,    // High if matrix is non-positive definite
    ...
    // Registers to hold decomposed L matrix and temporary y vector
    mat_t L;
    vec_t y;
    vec_t p_reg;
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            case (state)
                ...
                // =============================================================
                // PHASE 1: CHOLESKY FACTORIZATION (A = L · Lᵀ)
                // =============================================================
                CHOL_DIAG_SUM: begin
                    sum_acc = 32'sd0;
                ...
                // =============================================================
                // PHASE 2: FORWARD SUBSTITUTION (L · y = -g)
                // =============================================================
                CHOL_FWD_SUM: begin
                    sum_acc = 32'sd0;
                    for (int k = 0; k < MAX_VARS; k++) begin
                ...
                // =============================================================
                // PHASE 3: BACKWARD SUBSTITUTION (Lᵀ · p = y)
                // =============================================================
                CHOL_BWD_SUM: begin
                    sum_acc = 32'sd0;
                    for (int k = 0; k < MAX_VARS; k++) begin
