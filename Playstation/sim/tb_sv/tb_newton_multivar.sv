// =============================================================================
// File Name   : tb_newton_multivar.sv
// Module Name : tb_newton_multivar
// Project     : Universal Multivariable Newton 2nd-Order Accelerator
// -----------------------------------------------------------------------------
// Description:
//   Verification testbench for Multivariable N-Dimensional Newton Optimization.
//   Verifies convergence of coupled 2D and 3D non-linear equations in hardware.
// =============================================================================
...
module tb_newton_multivar;
    ...
    // Instantiate Multivariable Top SoC
    newton_multivar_top dut (
        .clk        (clk),
        .rst_n      (rst_n),
        .prog_en    (prog_en),
        .prog_addr  (prog_addr),
    ...
    // Task to program micro-instructions
    ...
    initial begin
        $display("==================================================================");
        $display(" Multivariable N-Dimensional Newton 2nd-Order Accelerator Testbench");
        $display("==================================================================");
        ...
        step_alpha = Q16_ONE;
        lambda_reg = 32'h0000_0100;     // damping ≈ 0.0039
        max_iters  = 8'd50;
        start      = 1'b1;
        @(posedge clk);
        start      = 1'b0;

        $display("Starting Multivariable Newton Hardware from initial guess (x0, x1) = (10.0, 10.0)...");
        @(posedge done);
        #1;
        ...
        if (status == STATUS_CONVERGED && q16_to_real(get_vec(x_optimal, 2'd0)) > -0.70 && q16_to_real(get_vec(x_optimal, 2'd0)) < -0.60 &&
            q16_to_real(get_vec(x_optimal, 2'd1)) > 5.25 && q16_to_real(get_vec(x_optimal, 2'd1)) < 5.40) begin
            $display("[TEST 1 PASSED] Successfully converged 2D coupled system in hardware!");
        end else begin
        ...
        @(posedge clk);
        num_vars   = 3'd2;
        x_init     = set_vec(set_vec(x_init, 2'd0, real_to_q16(8.0)), 2'd1, real_to_q16(9.0));
        tolerance  = 32'h0000_0080;
        step_alpha = Q16_ONE;
        lambda_reg = 32'h0000_0100;
        max_iters  = 8'd50;
        start      = 1'b1;
        @(posedge clk);
        start      = 1'b0;

        $display("Starting Multivariable Newton Hardware from initial guess (x0, x1) = (8.0, 9.0)...");
        @(posedge done);
        #1;
        ...
        if (status == STATUS_CONVERGED && q16_to_real(get_vec(x_optimal, 2'd0)) > 2.95 && q16_to_real(get_vec(x_optimal, 2'd0)) < 3.05 &&
            q16_to_real(get_vec(x_optimal, 2'd1)) > 3.95 && q16_to_real(get_vec(x_optimal, 2'd1)) < 4.05) begin
            $display("[TEST 2 PASSED] Successfully converged 2D paraboloid in hardware!");
        end else begin
        ...
        $display("Starting 3D Multivariable Newton Hardware from initial guess (5.0, 6.0, 7.0)...");
        @(posedge done);
        #1;
        ...
        if (status == STATUS_CONVERGED && q16_to_real(get_vec(x_optimal, 2'd0)) > -0.05 && q16_to_real(get_vec(x_optimal, 2'd0)) < 0.05 &&
            ...
            $display("[TEST 3 PASSED] Successfully converged 3D coupled system in hardware!");
        end else begin
            $display("[TEST 3 FAILED] Did not converge to target optimum.");
        end

        $display("\n==================================================================");
        $display(" ALL MULTIVARIABLE HARDWARE TESTS COMPLETED!");
        $display("==================================================================");
        #100;
        $finish;
    end
