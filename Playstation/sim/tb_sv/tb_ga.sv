...
// -----------------------------------------------------------------------------
// Description:
//   Comprehensive SystemVerilog testbench for Genetic Algorithm Accelerator.
//   Verifies:
//   1. 2D Decoupled Quadratic Global Minimization (Target: (3.0, 4.0))
//   2. 2D Non-Convex Curved Rosenbrock Valley (Target: (1.0, 1.0))
//   3. 3D Multi-Modal Fitness Optimization (Target: (0.0, 0.0, 0.0))
// =============================================================================
...
module tb_ga;
    ...
    initial begin
        $display("==================================================================");
        $display(" Genetic Algorithm (GA) Global Search TB (Solver #29)");
        $display("==================================================================");
        ...
        mut_prob        = real_to_q16(0.30);
        mut_scale       = real_to_q16(0.50);
        crossover_prob  = real_to_q16(0.85);
        ...
        $display("Starting Genetic Algorithm evolution loop...");
        @(posedge clk);
        opt_valid = 1'b1;
        @(posedge clk);
        opt_valid = 1'b0;
        ...
        if (fit_best < 0.50 && x0_best > 2.20 && x0_best < 3.80 && x1_best > 3.20 && x1_best < 4.80) begin
            $display("[TEST 1 PASSED] Successfully evolved population to global quadratic minimum!");
        end else begin
        ...
        $display("--> ROSENBROCK EVOLUTION RESULTS:");
        $display("    Completed Generations: %0d", gen_count);
        $display("    Best Solution x*:      (%f, %f)", x0_best, x1_best);
        $display("    Minimum Fitness F*:    %f", fit_best);

        if (fit_best < 0.20 && x0_best > 0.70 && x0_best < 1.30 && x1_best > 0.50 && x1_best < 1.60) begin
            $display("[TEST 2 PASSED] Successfully traversed Rosenbrock curved valley to global optimum!");
        end else begin
        ...
        if (fit_best < 0.10 && x0_best > -0.30 && x0_best < 0.30 &&
            ...
            $display("[TEST 3 PASSED] Successfully minimized 3D multi-modal fitness function!");
        end else begin
            $display("[TEST 3 FAILED] 3D fitness solution outside tolerance.");
        end

        $display("\n==================================================================");
        $display(" ALL GENETIC ALGORITHM (GA) HARDWARE OPTIMIZATION TESTS COMPLETED!");
        $display("==================================================================");
        #100;
        $finish;
    end
