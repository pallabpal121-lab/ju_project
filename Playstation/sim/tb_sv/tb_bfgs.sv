// =============================================================================
// File Name   : tb_bfgs.sv
// Module Name : tb_bfgs
// Project     : Quasi-Newton BFGS Optimization Accelerator (Solver #4)
// -----------------------------------------------------------------------------
// Description:
//   Comprehensive SystemVerilog testbench for the Quasi-Newton BFGS Accelerator.
//   Verifies convergence of coupled 2D and 3D non-linear equations via Rank-2
//   Inverse Hessian updates without computing explicit matrix inversions.
// =============================================================================
...
module tb_bfgs;
    ...
    initial begin
        $display("==================================================================");
        $display(" Quasi-Newton BFGS Optimization Accelerator Testbench (Solver #4)");
        $display("==================================================================");
        ...
        $display("Starting BFGS Optimizer from initial guess (x0, x1) = (8.0, 8.0)...");
        @(posedge done);
        #1;

        $display("--> BFGS SOLVER FINISHED!");
        ...
        if (status == STATUS_CONVERGED &&
            ...
            $display("[TEST 1 PASSED] Successfully converged 2D coupled system via BFGS!");
        end else begin
        ...
        step_alpha = Q16_HALF; // alpha = 0.5
        max_iters  = 8'd50;
        start      = 1'b1;
        @(posedge clk);
        start      = 1'b0;

        $display("Starting BFGS Optimizer from initial guess (x0, x1) = (10.0, 10.0)...");
        @(posedge done);
        #1;
        ...
        if (status == STATUS_CONVERGED &&
            ...
            $display("[TEST 2 PASSED] Successfully converged 2D paraboloid via BFGS!");
        end else begin
        ...
        $display("Starting 3D BFGS Optimizer from initial guess (5.0, 6.0, 7.0)...");
        @(posedge done);
        #1;
        ...
        if (status == STATUS_CONVERGED &&
            ...
            $display("[TEST 3 PASSED] Successfully converged 3D coupled system via BFGS!");
        end else begin
            $display("[TEST 3 FAILED] Did not converge to target optimum.");
        end

        $display("\n==================================================================");
        $display(" ALL BFGS HARDWARE TESTS COMPLETED!");
        $display("==================================================================");
        #100;
        $finish;
    end
