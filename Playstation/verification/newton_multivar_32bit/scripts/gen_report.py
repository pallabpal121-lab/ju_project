...
for log_path in test_logs:
    ...
    # Parse Coverage
    cov_match = re.search(r"Optimization Coverage\s*:\s*([\d\.]+)%", content)
...
with open(summary_file, "w") as f:
    ...
    f.write(f"{'TEST NAME':<32} | {'STATUS':<8} | {'MATCH':<5} | {'MISMATCH':<8} | {'COV %':<8} | {'SIM TIME':<10}\n")
    f.write("-" * 80 + "\n")
    for t in tests_data:
        f.write(f"{t['test']:<32} | {t['status']:<8} | {t['matches']:<5} | {t['mismatches']:<8} | {t['coverage']:<8} | {t['sim_time']:<10}\n")
    f.write("-" * 80 + "\n\n")
